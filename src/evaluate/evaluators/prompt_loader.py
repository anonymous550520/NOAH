def load_hallucination_prompt(recipe, result):
    ...
    prompt = (
        ...
        .replace("{INFERENCE_CAPTION}", caption.strip())
    )
...
def load_omission_prompt(recipe, result):
    ...
    template_path = "src/evaluate/prompts/omission_composite_prompt.txt"
    with open(template_path, "r") as f:
        template = f.read()
...
def load_omission_original_prompt(recipe, result):
    template_path = "src/evaluate/prompts/omission_original_prompt.txt"
    with open(template_path, "r") as f:
        template = f.read()
