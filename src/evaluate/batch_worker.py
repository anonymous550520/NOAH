...
def try_merge_joint(meta_path: Path, parsed_path: Path) -> None:
    ...
    mode = meta.get("mode")
    _orig = meta.get("original_mode")
    if isinstance(_orig, str):
        original_mode = _orig.lower() in ("true", "1")
    else:
        original_mode = bool(_orig) if _orig is not None else False
    ...
    for _id in merged_ids:
        ...
        if not original_mode and "inserted_omission_count" in oitem:
