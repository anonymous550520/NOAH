...
async def run_evaluation(mode, judge_model, recipes, results, output, max_concurrent, api_mode="sync", original_mode=False):
        ...
        def build_prompts(which: str):
            ...
            for id in ids:
                ...
                if which == "hallucination":
                    ...
                    if original_mode:
                        prompt = load_omission_original_prompt(recipe, result)
                    else:
                        prompt = load_omission_prompt(recipe, result)
                reqs.append((str(id), prompt))
            return reqs

        batch_extra_meta = {"original_mode": original_mode}
        print(f"[enqueue] mode={mode}, original_mode={original_mode}, api_mode={api_mode}")
        if mode in ("hallucination", "joint"):
            ...
            if h_reqs:
                h_handles = enqueue_chat_completions_batch(
                    ...
                    extra_metadata=batch_extra_meta,
                )
    ...
    # Evaluate
    print(f"Starting evaluation: mode = {mode}, original_mode = {original_mode}, api_mode = {api_mode}")
    if mode == "hallucination":
        results = await evaluate_hallucination_async(judge_model, recipes_lookup, results_lookup, max_concurrent)
    elif mode == "omission":
        results = await evaluate_omission_async(judge_model, recipes_lookup, results_lookup, max_concurrent, original_mode)
    elif mode == "joint":
        h_results = await evaluate_hallucination_async(judge_model, recipes_lookup, results_lookup, max_concurrent)
        o_results = await evaluate_omission_async(judge_model, recipes_lookup, results_lookup, max_concurrent, original_mode)
    ...
    # Compute metrics
    n = len(results)
    sum_ehr = sum_eor = sum_ieor = 0.0
    sum_chr = sum_cor = 0.0
    for e in results:
        h = e["hallucination"]
        o = e["omission"]

        ehr = h["hallucination_count"] / h["extracted_events_count"] if h["extracted_events_count"] > 0 else 0.0
        sum_chr += 1.0 if h["hallucination_count"] > 0 else 0.0
        sum_cor += 1.0 if o["total_omission_count"] > 0 else 0.0

        if original_mode:
            n_gt = o["ground_truth_events_count"] if o["ground_truth_events_count"] > 0 else len(e["ground_truth"])
            eor = o["total_omission_count"] / n_gt if n_gt > 0 else 0.0
        else:
            n_gt = len(e["ground_truth"])
            denom = (n_gt - 1) if n_gt > 1 else 1
            inserted = o.get("inserted_omission_count", 0)
            if inserted > 0:
                eor = (o["total_omission_count"] - 1) / denom
            else:
                eor = o["total_omission_count"] / denom
            sum_ieor += 1.0 if inserted > 0 else 0.0

        sum_ehr += ehr
        sum_eor += eor

    metric = {
        "CHR": round(sum_chr / n, 4),
        "EHR": round(sum_ehr / n, 4),
        "COR": round(sum_cor / n, 4),
        "EOR": round(sum_eor / n, 4),
    }
    if not original_mode:
        metric["IEOR"] = round(sum_ieor / n, 4)
    print(f"Metrics: {metric}")

    # Save
    print(f"Saving to {output}")
    save_json({"metric": metric, "results": results}, output)
    print(f"Done. Total: {n} evaluated.")
...
def parse_args():
    ...
    arg.add_argument("--original", action="store_true", help="Use original video evaluation mode (no inserted events in metadata)")
...
def main():
    args = parse_args()
    asyncio.run(run_evaluation(args.mode, args.judge_model, args.recipes, args.results, args.output, args.max_concurrent, args.api_mode, args.original))
