...
#   CUDA_VISIBLE_DEVICES=0 ./scripts/run_captioning.sh MODEL_NAME
# example:
...
SAVE_DIR="outputs/captions/${MODEL_NAME}/run_${TIMESTAMP}"
mkdir -p "$SAVE_DIR"
...
echo "Using GPUs: [${GPUS_TO_USE[*]}], num_shards=${NUM_SHARDS}"

SHARD_IDX=0
for GPU_ID in "${GPUS_TO_USE[@]}"; do
  echo "  -> Launching shard ${SHARD_IDX} (gpu=${GPU_ID})"
  CUDA_VISIBLE_DEVICES="$GPU_ID" python src/run_captioning.py \
